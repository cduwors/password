...
function askQuestions() {
	...
	while (!correctInput) {
		charLength = window.prompt(
			"How long do you want your password? Enter a number between 8 and 128."
		);
		...
		//if yes (true), go to next question
			...
			window.alert(
				"Answer the following questions for what you would like included in your password. You MUST include at least one of the following 4 options."
			);
			...
			while (!isGood) {
				...
				if (
					...
					window.alert(
						"Invalid input. You MUST choose at least one character type from the 4 options. Please try again"
					);
				} else {
					isGood = true;
				}
...
function charConfirm() {
	...
	if (lowerLetters) {
		window.alert(
			"You have chosen to include LOWERCASE letters in your Password"
		);
	} else {
		window.alert(
			"You have chosen NOT to include lowercase letters in your Password"
		);
	}
	...
	if (upperLetters) {
		window.alert(
			"You have chosen to include UPPERCASE letters in your Password"
		);
	} else {
		window.alert(
			"You have chosen NOT to include uppercase letters in your Password"
		);
	}
	...
	specials = window.confirm(
		"Do you want special characters included? (e.g. #, $, %, &, etc.)"
	);

	if (specials) {
		window.alert(
			"You have chosen to include SPECIAL CHARACTERS in your Password"
		);
	} else {
		window.alert(
			"You have chosen NOT to include special characters in your Password"
		);
	}
}
